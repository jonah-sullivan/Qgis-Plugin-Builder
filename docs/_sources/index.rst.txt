...
#. If automatic compilation failed during plugin generation, manually compile your resource file using pyrcc6
#. Install the plugin
#. Test it
...
When you click **Generate**, Plugin Builder will attempt to build your ``resources.py`` file using ``pyrcc6``.

If ``pyrcc6`` is not found in your path, you'll see:
...
Plugin Builder attempts to compile it for you during generation, but it that fails (i.e. ``pyrcc6`` isn't found), you'll
...
To compile the resource file into Python code, use the ``pyrcc6`` utility
